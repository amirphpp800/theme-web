...
const wallpaperFileTypeSelect = document.getElementById('wallpaper-file-type');
const downloadUrlSection = document.getElementById('download-url-section');
...
function setupEventListeners() {
    ...
    // Wallpaper file type change
    if (wallpaperFileTypeSelect) {
        wallpaperFileTypeSelect.addEventListener('change', toggleDownloadOptions);
    }
...
function toggleDownloadOptions() {
    const fileType = wallpaperFileTypeSelect.value;
    if (fileType === 'zip') {
        downloadUrlSection.classList.remove('hidden');
        fileSizeSection.classList.remove('hidden');
    } else {
        downloadUrlSection.classList.add('hidden');
        fileSizeSection.classList.add('hidden');
    }
}
...
async function handleAddPrompt(event) {
    ...
    if (activeTab === 'url') {
        ...
        try {
            // Convert to base64 then upload to server KV to avoid embedding large data in lists
            ...
            const uploadResp = await fetch('/api/admin/upload', {
                ...
                body: JSON.stringify({ imageData: base64Data, fileName: file.name, type: 'prompt' })
            });
            const uploadJson = await uploadResp.json();
            if (!uploadResp.ok || !uploadJson.success) {
                alert(uploadJson.error || 'خطا در آپلود فایل!');
                return;
            }
            imageSource = uploadJson.imageUrl; // Store served URL instead of base64
        } catch (error) {
...
async function handleAddWallpaper(event) {
    ...
    if (activeTab === 'url') {
        ...
        try {
            ...
            const uploadResp = await fetch('/api/admin/upload', {
                ...
                body: JSON.stringify({ imageData: base64Data, fileName: file.name, type: 'wallpaper' })
            });
            const uploadJson = await uploadResp.json();
            if (!uploadResp.ok || !uploadJson.success) {
                alert(uploadJson.error || 'خطا در آپلود فایل!');
    ...
    const fileType = document.getElementById('wallpaper-file-type').value;
    const downloadUrl = document.getElementById('wallpaper-download-url').value;
    const fileSize = document.getElementById('wallpaper-file-size').value;
    
    const wallpaperData = {
        title: {
            fa: document.getElementById('wallpaper-title-fa').value.trim(),
            en: document.getElementById('wallpaper-title-en').value.trim()
        },
        image: imageSource,
        imageType: activeTab, // 'url' or 'upload'
        type: wallpaperType,
        ...
        fileSize: fileSize ? parseFloat(fileSize) * 1024 * 1024 : null, // Convert MB to bytes
        price: price ? {
            fa: `${price} تومان`,
            en: `${price} IRR`
        } : null
    ...
    wallpapers.forEach(wallpaper => {
        ...
        wallpaperElement.innerHTML = `
            ...
            <div class="flex items-center justify-between mb-2">
                ...
                <span class="text-xs ${wallpaper.fileType === 'zip' ? 'text-blue-600' : 'text-gray-600'} font-medium">
                    ${wallpaper.fileType === 'zip' ? 'ZIP' : 'تصویر'}
                </span>
            </div>
            <div class="flex justify-between items-center">
                ${wallpaper.fileSize ? `<span class="text-xs text-gray-500">${(wallpaper.fileSize / (1024 * 1024)).toFixed(1)} MB</span>` : '<span></span>'}
...
function handleFileSelect(event, type) {
    ...
    // Validate file type
    const allowedTypes = type === 'wallpaper' 
    ...
    if (!allowedTypes.includes(file.type) && !file.name.toLowerCase().endsWith('.zip')) {
        const typeText = type === 'wallpaper' ? 'تصویری یا ZIP' : 'تصویری';
        alert(`لطفاً فقط فایل ${typeText} انتخاب کنید!`);
        return;
    }
    
    // Remove size limit for wallpapers to support large ZIP files
    const maxSize = type === 'wallpaper' ? 50 * 1024 * 1024 : 5 * 1024 * 1024; // 50MB for wallpapers, 5MB for prompts
...
function showFilePreview(file, type) {
    ...
    reader.onload = function(e) {
        ...
        if (uploadArea && preview && previewImg && fileName) {
...
// Password visibility toggle function for admin panel - Enhanced
function setupPasswordToggle() {
    ...
    const newEyeClosed = newToggleBtn.querySelector('#admin-eye-closed');
    const newEyeOpen = newToggleBtn.querySelector('#admin-eye-open');
